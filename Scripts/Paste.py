...
if Config["Settings"]["Spotify_Logo"]:
...
if Config["Image"]["Background_Color_to_Remove"] != None:
	Color = tuple(int(Config["Image"]["Background_Color_to_Remove"][1:].upper()[x:x+2], 16) for x in (0, 2, 4))
	Data = Artist.getdata()
	Data_New = []
...
Background = Background.resize((Config["Image"]["Max_Image_Height"],) * 2, Image.LANCZOS)
Background = Background.crop(Background.getbbox())

#---#

Text_1 = Trans_Paste(Text_1, Gradient, (0, - Gradient.size[1] + Image_Base.size[0]))
Text_1 = Trans_Paste(Text_1, Background, ((Image_Base.size[0] - Background.size[0]) // 2, - Background.size[1] + Config["Image"]["Bottom_Offset"] + Image_Base.size[0]))